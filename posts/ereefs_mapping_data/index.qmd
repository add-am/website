...
Okay so I know we have already mapped the data a bunch of times above, but I would like to explore the visuals just a little further before we round out this blog. Specifically, I would like to add some visual cues to provide a reference point. These include:
...
The first thing I'd like to explore is how a continuous scale looks:

```{r}

#create a simple plot of the data
tm_shape(agg_example_2) +
  tm_raster(col.scale = tm_scale_continuous(values = my_pal),
                                           #label.format = list(digits = 2)),
            col.legend = tm_legend(reverse = T)) +
  tm_shape(dt_region) +
...
Is that better? Hard to tell honestly. However I would then like to see if a log scale helps:

```{r}

#create a simple plot of the data
tm_shape(agg_example_2) +
  tm_raster(col.scale = tm_scale_continuous_log10(values = my_pal),
            col.legend = tm_legend(reverse = T,
                                   title = "Chla (ug/L)")) +
  tm_shape(dt_region) +
  tm_polygons(fill = NULL,
