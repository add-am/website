...
The equation simply represents the line that best fits all of the points in a dataset. Where the dataset contains one indepent variable (x) and one dependent variable (y). The idea of linear regression two fold:

 1. Establish if there is a relationship between x and y, and
 2. Use the equation to predict unknown y values based on x

Multiple linear regression is the natural extension of this equation. The only thing that changes is there is now an additional independent variable, or several additional independent variables. The equation for multiple linear regression looks like this:

$y_{i} = \beta_{0} + \beta_{1}*x_{i,1}+\beta_{2}*x_{i,2}+...+\beta_{n}*x_{i,n}$

Again, I'll acknowledge that mathemeticians must have been on crack when they decided how to write these things. Anyway, it is essentially the same thing as above, there are now just more independent variables. Some important notes about this equation:
 ...
 - To predict y we input several independent variables, these are the $x_{i,n}$ terms.
    - The "i" just means that each group of independent variables we plug in will return 1 y value. (This makes sense later)
    - The "n" is just signifying that each "x" is unique. Note in the full equation how the first "x" is 1, then the second "x" is 2 and so on. This is important because it signifies that each of the x terms is going to be a different value.
...
The first thing we need to do is build a dataset that explores this relationship mouse weight, body length and tail length. I am going to manually create 15 of each of these values. Note that we want to show a relationship so I will deliberately try to add a trend, however to make things interesting I will make sure it is not a completely perfect reationship. We will achieve this by asking for random numbers between 0g and 1000g for weight, 0cm and 30cm for the body, and 0cm to 25cm for the tail. Then each of these will be ordered smallest to largest to create a trend:
...
weight <- runif(15)*1000
body_length <- runif(15)*30
tail_length <- runif(15)*25

#create an example dataset, note that we are being very delibrate to establish a trend by ordering each var from small to large
mouse_df <- data.frame(
...
However something to note here (that is particularly important when doing this type of analysis for real) is that the two independent variables (body length and tail length) seem to also be highly correlated. This can introduce problems in your model for a few reasons:
 ...
 You can address collinearity in a few different ways:
  ...
  - If it affects variables you are not going to use in the final model you can also ignore it (or apparently if it affects control variables - i am not sure i understand that one yet)
