...
As we noted above, the data extends outside our area of interest for two main reasons: 
...
notes:
 - we can still use st_crop on curvilinear, but we cant drop NA values it seems
 - we can drop NA in a regular grid I think - happens automatically with st_crop?
...
cropped_data <- extracted_data |> 
  st_transform("EPSG:7844") |>
  st_crop(dt_region) |> 
  st_as_stars()

#create a simple map
tm_shape(cropped_test) +
...
However an interesting side point to make here is 
...
#crop to the actual area of interest
cropped_test <- warped_data |> 
  #st_transform() |>
  st_crop(dt_region) #|> 
  st_as_stars()

down_sample <- st_downsample(extracted_data, 1)
